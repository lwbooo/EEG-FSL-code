    ...
    def train_baseline():
        ...
        for time in range(N_times):
            for fold in  range(N_split):
                ...
                model = xgb.XGBRegressor(booster = 'gbtree',learning_rate = lr,n_estimators = es,eval_metric=metric,
        ...
        for gamma in gamma_list:
            for subsample in subsample_list:
                for colsample in colsample_list:
                    baseline_acc,withpsd_acc = XGB(0.01,es,'rmse',3,weight,gamma,subsample,colsample,0,0)
                    ...
                    if best_avg_bas > np.mean(baseline_acc):
                        ...
                        best_para_bas ={'lr':0.01,'es':es,'metric':'rmse','depth':3,'weight':weight,
                                                                    'gamma':gamma,'subsample':subsample,'colsample':colsample,'alpha':0,'lambda':0}
                    if best_avg_psd > np.mean(withpsd_acc):
                        best_avg_psd = np.mean(withpsd_acc)
                        best_all_psd = withpsd_acc
                        best_para_psd ={'lr':0.01,'es':es,'metric':'rmse','depth':3,'weight':weight,
                                                                    'gamma':gamma,'subsample':subsample,'colsample':colsample,'alpha':0,'lambda':0}
